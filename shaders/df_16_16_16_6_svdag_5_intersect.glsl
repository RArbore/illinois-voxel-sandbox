...
bool intersect_format_1(uint volume_id, uint node_id, vec3 obj_ray_pos, vec3 obj_ray_dir, vec3 lower, float o_t, uint hit_kind) {
    ...
    ivec3 chunk_ray_voxel = ivec3(min(chunk_ray_intersect_point, ivec3(31, 31, 31)));
    ...
    while (all(greaterThanEqual(chunk_ray_voxel, ivec3(0))) && all(lessThan(chunk_ray_voxel, ivec3(32, 32, 32)))) {
        uint32_t child_id = lookup_utility_restart_svdag(volume_id, node_id, 5, chunk_ray_voxel);
