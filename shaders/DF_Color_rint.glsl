...
void main() {
	...
	while (all(greaterThanEqual(obj_ray_voxel, ivec3(0))) && all(lessThan(obj_ray_voxel, volume_size))) {
	    if (budget == 0) {
		uint32_t voxel_index = linearize_index(obj_ray_voxel, volume_size.x, volume_size.y, volume_size.z);
		uint palette = uint(raw_buffers[volume_id].voxel_colors[voxel_index].alpha_);
