...
bool intersect_format_0(uint volume_id, uint node_id, vec3 obj_ray_pos, vec3 obj_ray_dir, vec3 lower, float o_t, uint hit_kind) {
    ...
    ivec3 chunk_ray_voxel = ivec3(min(chunk_ray_intersect_point, ivec3(14, 6, 8)));
    ...
    uint budget = 0;
    while (all(greaterThanEqual(chunk_ray_voxel, ivec3(0))) && all(lessThan(chunk_ray_voxel, ivec3(15, 7, 9)))) {
        if (budget == 0) {
            uint32_t voxel_index = linearize_index(chunk_ray_voxel, 15, 7, 9);
...
void main() {
    ...
    aabb_intersect_result r = hit_aabb(vec3(0.0), vec3(1920.0, 896.0, 1152.0), obj_ray_pos, obj_ray_dir);
