...
bool intersect_format_2(uint volume_id, uint node_id, vec3 obj_ray_pos, vec3 obj_ray_dir, vec3 lower, float o_t, uint hit_kind) {
    ...
    ivec3 chunk_ray_voxel = ivec3(min(chunk_ray_intersect_point, ivec3(7, 7, 7)));
    ...
    while (all(greaterThanEqual(chunk_ray_voxel, ivec3(0))) && all(lessThan(chunk_ray_voxel, ivec3(8, 8, 8)))) {
        uint32_t child_id = lookup_utility_restart_svdag(volume_id, node_id, 3, chunk_ray_voxel);
...
bool intersect_format_1(uint volume_id, uint node_id, vec3 obj_ray_pos, vec3 obj_ray_dir, vec3 lower, float o_t, uint hit_kind) {
    ...
    while (all(greaterThanEqual(chunk_ray_voxel, ivec3(0))) && all(lessThan(chunk_ray_voxel, ivec3(8, 8, 8)))) {
        if (budget == 0) {
            uint32_t voxel_index = linearize_index(chunk_ray_voxel, 8, 8, 8);
            uint32_t child_id = voxel_buffers[volume_id].voxels[node_id + voxel_index] & 268435455;
            budget = (voxel_buffers[volume_id].voxels[node_id + voxel_index] >> 28) + 1;
