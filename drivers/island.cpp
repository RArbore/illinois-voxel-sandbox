...
int main(int argc, char *argv[]) {
    ...
    // std::vector<std::string> model_paths = {"/white_oak_1", "/wheat01",
    // "/stones002", "/grass", "/basalt"}; for (uint32_t i = 0; i <
    // model_paths.size(); ++i) {
    ...
    tree_stream.read(reinterpret_cast<char *>(tree_svdag.data()),
                     tree_file_size);
    std::cout << "Tree SVDAG Size: " << tree_svdag.size() << "\n";

    uint32_t *tree_svdag_ptr = reinterpret_cast<uint32_t *>(tree_svdag.data());
    uint32_t chunk_width = tree_svdag_ptr[0], chunk_height = tree_svdag_ptr[1],
    ...
    for (uint32_t i = 0; i < 10; i++) {
        ...
        float offset = noise.GetNoise((r * cos(theta) + 150) / 10,
                                      (r * sin(theta) + 150) / 10) *
                       10;
        ...
        glm::mat3x3 scale = {scaleFactor, 0.0F, 0.0F, 0.0F,       scaleFactor,
    ...
    std::ifstream stone_stream(stone_model_path,
                               std::ios::in | std::ios::binary);
    const auto stone_file_size = std::filesystem::file_size(stone_model_path);
    std::vector<std::byte> stone_svdag =
        std::vector<std::byte>(stone_file_size);
    stone_stream.read(reinterpret_cast<char *>(stone_svdag.data()),
                      stone_file_size);
    std::cout << "Stone SVDAG Size: " << stone_svdag.size() << "\n";

    uint32_t *stone_svdag_ptr =
        reinterpret_cast<uint32_t *>(stone_svdag.data());
    chunk_width = stone_svdag_ptr[0], chunk_height = stone_svdag_ptr[1],
    ...
    for (uint32_t i = 0; i < 10; i++) {
        for (uint32_t j = 0; j < 10; j++) {
            float offset =
                noise.GetNoise((static_cast<float>(i) * 60 - 200) / 10,
                               (static_cast<float>(j) * 60 - 200) / 10) *
                10;
            ...
            float scaleFactor = size;
            glm::mat3x3 scale = {stone_size, 0.0F, 0.0F, 0.0F,      stone_size,
                                 0.0F,       0.0F, 0.0F, stone_size};
            glm::mat3x3 transform = scale * rotationy;
            glm::mat3x4 cast_transform = glm::mat3x4(transform);
            cast_transform =
                cast_transform + stone_transform + custom_stone_transform;
            auto stone_object =
                build_object(context, stone_model, cast_transform);
    ...
    std::ifstream wheat_stream(wheat_model_path,
                               std::ios::in | std::ios::binary);
    const auto wheat_file_size = std::filesystem::file_size(wheat_model_path);
    std::vector<std::byte> wheat_svdag =
        std::vector<std::byte>(wheat_file_size);
    wheat_stream.read(reinterpret_cast<char *>(wheat_svdag.data()),
                      wheat_file_size);
    std::cout << "Wheat SVDAG Size: " << wheat_svdag.size() << "\n";

    uint32_t *wheat_svdag_ptr =
        reinterpret_cast<uint32_t *>(wheat_svdag.data());
    chunk_width = wheat_svdag_ptr[0], chunk_height = wheat_svdag_ptr[1],
    ...
    for (uint32_t i = 0; i < 5000; i++) {
        ...
        float r = radius * sqrt((rand() % 1000) / 1000.0F);
        float theta = ((rand() % 1000) / 1000.0F) * 2 * 3.14159265358979323846;
        float offset = noise.GetNoise((r * cos(theta) + 150) / 1,
                                      (r * sin(theta) + 150) / 1) *
                       10;
        glm::mat3x4 randomtranslation = {0.0F, 0.0F, 0.0F, r * cos(theta),
                                         0.0F, 0.0F, 0.0F, 0,
