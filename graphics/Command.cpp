...
CommandPool::CommandPool(std::shared_ptr<Device> device) {
    ...
    ASSERT(vkCreateCommandPool(device->get_device(), &create_info, nullptr,
                               &command_pool_),
...
VkCommandPool CommandPool::get_pool() { return command_pool_; }
...
Command::Command(std::shared_ptr<CommandPool> command_pool) {
    ...
    ASSERT(vkAllocateCommandBuffers(command_pool->get_device()->get_device(),
                                    &allocate_info, &buffer_),
           "Unable to create command buffers.");

    command_pool_ = command_pool;
}

VkCommandBuffer Command::get_buffer() { return buffer_; }
