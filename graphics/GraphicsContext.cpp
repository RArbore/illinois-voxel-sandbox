...
GraphicsContext::GraphicsContext(std::shared_ptr<Window> window) {
    ...
    ring_buffer_ =
        std::make_shared<RingBuffer>(gpu_allocator_, command_pool_, (1 << 30) + (1 << 29));
    ...
    for (uint32_t i = 0; i < customs.size(); ++i) {
        custom_intersection_shaders_.emplace(
            customs[i], i + FORMAT_TO_SBT_OFFSET.size() + 1);
    }
...
build_model(std::shared_ptr<GraphicsContext> context, VoxelChunkPtr chunk) {
    ...
    uint32_t sbt_offset =
        chunk->get_custom_format().empty()
            ? FORMAT_TO_SBT_OFFSET.at(
                  {chunk->get_format(), chunk->get_attribute_set()})
            : context->custom_intersection_shaders_.at(
                  chunk->get_custom_format() + "_intersect");
...
void GraphicsContext::bind_scene_descriptors(
    ...
    builder.bind_buffer(2, chunk_dimensions_info,
                        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                        VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR |
                            VK_SHADER_STAGE_INTERSECTION_BIT_KHR);
