...
choose_swapchain_options(VkPhysicalDevice physical_device,
...
Swapchain::Swapchain(std::shared_ptr<Device> device,
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(device->get_physical_device(),
                                         ...
                                         nullptr);
    vkGetPhysicalDeviceSurfacePresentModesKHR(device->get_physical_device(),
                                              device->get_surface(),
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        device->get_physical_device(), device->get_surface(), &capabilities);
    vkGetPhysicalDeviceSurfaceFormatsKHR(device->get_physical_device(),
                                         device->get_surface(), &num_formats,
                                         &formats.at(0));
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        device->get_physical_device(), device->get_surface(),
        &num_present_modes, &present_modes.at(0));

    const auto [surface_format, present_mode, swap_extent] =
        choose_swapchain_options(device->get_physical_device(), capabilities,
                                 formats, present_modes, window->get_window());
    ...
    ASSERT(vkCreateSwapchainKHR(device->get_device(), &create_info, nullptr,
                                &swapchain_),
    ...
    for (uint32_t image_index = 0; image_index < image_count; ++image_index) {
        ...
        ASSERT(vkCreateImageView(device->get_device(), &create_info, nullptr,
...
void Swapchain::present_image(uint32_t image_index,
...
choose_swapchain_options(VkPhysicalDevice physical_device,
    ...
    ASSERT(format_index < formats.size(),
           "ERROR: Queried surface format not found to be available.");

    present_mode = VK_PRESENT_MODE_FIFO_KHR;
    for (uint32_t present_mode_index = 0;
         present_mode_index < present_modes.size(); ++present_mode_index) {
        if (present_modes[present_mode_index] ==
            VK_PRESENT_MODE_IMMEDIATE_KHR) {
            present_mode = VK_PRESENT_MODE_IMMEDIATE_KHR;
        } else if (present_modes[present_mode_index] ==
    ...
    if (capabilities.currentExtent.width != UINT32_MAX) {
        ...
        swap_extent.width =
            swap_extent.width < capabilities.minImageExtent.width
                ? capabilities.minImageExtent.width
                : swap_extent.width;
        swap_extent.height =
            swap_extent.height < capabilities.minImageExtent.height
                ? capabilities.minImageExtent.height
                : swap_extent.height;
        swap_extent.width =
            swap_extent.width > capabilities.maxImageExtent.width
                ? capabilities.maxImageExtent.width
                : swap_extent.width;
        swap_extent.height =
            swap_extent.height > capabilities.maxImageExtent.height
