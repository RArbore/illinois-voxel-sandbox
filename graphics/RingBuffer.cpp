...
void RingBuffer::copy_to_device(
    ...
    if (blocked) {
        ...
        vkWaitForFences(device_->get_device(), 1, &vk_fence, VK_TRUE,
                        UINT64_MAX);
...
void RingBuffer::copy_to_device(
    ...
    if (!wraparound_necessary) {
        ...
        /*void *const first_dst_ptr = buffer_map_.data() + (virtual_counter_ %
        size); void *const second_dst_ptr = buffer_map_.data(); const
        VkDeviceSize first_size = size - (virtual_counter_ % size); const
        VkDeviceSize second_size = src.size() - first_size;
        memcpy(first_dst_ptr, src.data(), first_size);
        memcpy(second_dst_ptr, src.data() + first_size, second_size);
