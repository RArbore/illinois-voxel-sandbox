...
void RingBuffer::copy_to_device(std::shared_ptr<GPUVolume> dst,
	...
	Barrier layout_barrier(device_,
	...
	command->record([&](VkCommandBuffer command) {
	    ...
	    vkCmdCopyBufferToImage(command, buffer_->get_buffer(), dst->get_image(), dst_layout, 1, &copy);
	});
	device_->submit_command(command, wait_semaphores, signal_semaphores, fence);
