...
Fence::Fence(std::shared_ptr<Device> device, bool create_signaled) {
    ...
    ASSERT(vkCreateFence(device->get_device(), &fence_info, nullptr, &fence_),
...
Semaphore::Semaphore(std::shared_ptr<Device> device, bool timeline) {
    ...
    ASSERT(vkCreateSemaphore(device->get_device(), &semaphore_info, nullptr,
                             &semaphore_),
...
VkSemaphore Semaphore::get_semaphore() { return semaphore_; }
...
void Semaphore::wait() {
    ...
    wait_info.pSemaphores = &semaphore_;
    wait_info.pValues = &wait_value_;
    ASSERT(vkWaitSemaphores(device_->get_device(), &wait_info, UINT64_MAX), "Failed to wait on semaphore.");
}
...
void Semaphore::increment() {
    ASSERT(timeline_,
           "Must be a timeline semaphore to set a increment timeline.");
    ++wait_value_;
    ++signal_value_;
}

Barrier::Barrier(std::shared_ptr<Device> device,
                 VkPipelineStageFlags2 src_stages, VkAccessFlags2 src_accesses,
...
Barrier::Barrier(std::shared_ptr<Device> device,
    ...
    device_ = device;
    add(src_stages, src_accesses, dst_stages, dst_accesses, buffer, offset,
        size);
}
...
Barrier::Barrier(std::shared_ptr<Device> device,
    ...
    device_ = device;
    add(src_stages, src_accesses, dst_stages, dst_accesses, image, old_layout,
        new_layout, range);
}

void Barrier::add(VkPipelineStageFlags2 src_stages, VkAccessFlags2 src_accesses,
                  VkPipelineStageFlags2 dst_stages,
                  VkAccessFlags2 dst_accesses) {
...
void Barrier::record(VkCommandBuffer command) {
    ...
    dependency_info.memoryBarrierCount =
        static_cast<uint32_t>(global_barriers_.size());
