...
static std::vector<uint32_t> df_16_16_16_6_svdag_8_construct_node(Voxelizer &voxelizer, std::pair<std::ofstream &, uint32_t &> buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);

static std::array<uint32_t, 8> svdag_8_construct_node(Voxelizer &voxelizer, std::pair<std::ofstream &, uint32_t &> buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty, std::unordered_map<std::array<uint32_t, 8>, uint32_t> &deduplication_map);
...
void df_16_16_16_6_svdag_8_construct(Voxelizer &voxelizer, std::ofstream &buffer) {
    ...
    ++size;
    auto root_node = df_16_16_16_6_svdag_8_construct_node(voxelizer, {buffer, size}, 0, 0, 0, is_empty);
