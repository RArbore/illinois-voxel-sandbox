...
static std::array<uint32_t, 8> svdag_5_raw_16_16_16_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty, std::unordered_map<std::array<uint32_t, 8>, uint32_t> &deduplication_map);
...
std::vector<uint32_t> svdag_5_raw_16_16_16_construct(Voxelizer &voxelizer) {
    ...
    auto root_node = svdag_5_raw_16_16_16_construct_node(voxelizer, buffer, 0, 0, 0, is_empty, deduplication_map);
