...
static std::vector<uint32_t> df_256_256_256_12_svdag_4_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);

static std::array<uint32_t, 8> svdag_4_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty, std::map<std::array<uint32_t, 8>, uint32_t> &deduplication_map);
...
std::vector<uint32_t> df_256_256_256_12_svdag_4_construct(Voxelizer &voxelizer) {
    ...
    bool is_empty;
    auto root_node = df_256_256_256_12_svdag_4_construct_node(voxelizer, buffer, 0, 0, 0, is_empty);
...
static std::vector<uint32_t> df_256_256_256_12_svdag_4_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty) {
    ...
    is_empty = true;
    uint32_t k = 12;
    for (uint32_t g_z = 0; g_z < 256; ++g_z) {
        for (uint32_t g_y = 0; g_y < 256; ++g_y) {
            for (uint32_t g_x = 0; g_x < 256; ++g_x) {
    ...
    for (uint32_t g_z = 0; g_z < 256; ++g_z) {
        for (uint32_t g_y = 0; g_y < 256; ++g_y) {
            for (uint32_t g_x = 0; g_x < 256; ++g_x) {
                uint32_t g_voxel_offset = (g_x + g_y * 256 + g_z * 256 * 256);
                uint32_t min_dist = k;
                for (uint32_t kz = g_z > k ? g_z - k : 0; kz <= g_z + k && kz < 256; ++kz) {
                    for (uint32_t ky = g_y > k ? g_y - k : 0; ky <= g_y + k && ky < 256; ++ky) {
                        for (uint32_t kx = g_x > k ? g_x - k : 0; kx <= g_x + k && kx < 256; ++kx) {
                            size_t k_voxel_offset = kx + ky * 256 + kz * 256 * 256;
