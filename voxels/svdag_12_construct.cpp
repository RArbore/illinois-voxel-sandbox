...
static std::array<uint32_t, 8> svdag_12_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);
...
std::vector<uint32_t> svdag_12_construct(Voxelizer &voxelizer) {
    ...
    bool is_empty;
    auto root_node = svdag_12_construct_node(voxelizer, buffer, 0, 0, 0, is_empty);
