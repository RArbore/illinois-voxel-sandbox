    ...
    if (state_ == State::Disk && !uploading_ && !downloading_) {
        ...
        case Format::Raw: {
            ...
            ring_buffer->copy_to_device(volume_data_, VK_IMAGE_LAYOUT_GENERAL,
                                        get_cpu_data(), {timeline_},
                                        {timeline_});
            break;
        }
        case Format::SVO:
        case Format::SVDAG: {
            buffer_data_ = std::make_shared<GPUBuffer>(
                allocator, get_cpu_data().size(), 8,
...
void VoxelChunk::debug_print() {
    ...
    if (volume_data_) {
        std::cout << "  volume_data_: GPUVolume with dimensions ("
                  << volume_data_->get_extent().width << ", "
                  << volume_data_->get_extent().height << ", "
                  << volume_data_->get_extent().depth << ")\n";
    } else {
