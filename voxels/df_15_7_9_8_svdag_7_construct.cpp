...
static std::vector<uint32_t> df_15_7_9_8_svdag_7_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);

static std::array<uint32_t, 8> svdag_7_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);
...
std::vector<uint32_t> df_15_7_9_8_svdag_7_construct(Voxelizer &voxelizer) {
    ...
    bool is_empty;
    auto root_node = df_15_7_9_8_svdag_7_construct_node(voxelizer, buffer, 0, 0, 0, is_empty);
...
static std::vector<uint32_t> df_15_7_9_8_svdag_7_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty) {
    ...
    is_empty = true;
    uint32_t k = 8;
    for (uint32_t g_z = 0; g_z < 9; ++g_z) {
        for (uint32_t g_y = 0; g_y < 7; ++g_y) {
            for (uint32_t g_x = 0; g_x < 15; ++g_x) {
                uint32_t sub_lower_x = lower_x + g_x * 128;
                uint32_t sub_lower_y = lower_y + g_y * 128;
                uint32_t sub_lower_z = lower_z + g_z * 128;
                bool sub_is_empty;
                auto sub_chunk = svdag_7_construct_node(voxelizer, buffer, sub_lower_x, sub_lower_y, sub_lower_z, sub_is_empty);
                if (sub_is_empty) {
    ...
    for (uint32_t g_z = 0; g_z < 9; ++g_z) {
        for (uint32_t g_y = 0; g_y < 7; ++g_y) {
            for (uint32_t g_x = 0; g_x < 15; ++g_x) {
                uint32_t g_voxel_offset = (g_x + g_y * 15 + g_z * 15 * 7);
            uint32_t min_dist = k;
            for (uint32_t kz = g_z > k ? g_z - k : 0; kz <= g_z + k && kz < 9; ++kz) {
                for (uint32_t ky = g_y > k ? g_y - k : 0; ky <= g_y + k && ky < 7; ++ky) {
                    for (uint32_t kx = g_x > k ? g_x - k : 0; kx <= g_x + k && kx < 15; ++kx) {
                        size_t k_voxel_offset = kx + ky * 15 + kz * 15 * 7;
