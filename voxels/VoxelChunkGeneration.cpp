...
std::vector<std::byte> generate_basic_procedural_chunk(uint32_t width,
                                                       uint32_t height,
                                                       uint32_t depth) {
    std::vector<std::byte> data(static_cast<size_t>(width) * static_cast<size_t>(height) * static_cast<size_t>(depth) * 4,
                                static_cast<std::byte>(0));
    FastNoiseLite noise;
    ...
    for (uint32_t x = 0; x < width; ++x) {
        for (uint32_t y = 0; y < height; ++y) {
            for (uint32_t z = 0; z < depth; ++z) {
                if (noise.GetNoise(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z)) > 0.0F) {
