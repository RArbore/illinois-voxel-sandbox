...
test_loader(const std::string &filepath, ChunkManager &chunk_manager,
    ...
    std::shared_ptr<GraphicsObject> obj =
        build_object(context, graphicsmodel, glmtransform);
...
load_vox_scene(const std::string &filepath, ChunkManager &chunk_manager,
        ...
        for (uint32_t y = 0; y < model->size_y; y++) {
            for (uint32_t z = 0; z < model->size_z; z++) {
                for (uint32_t x = 0; x < model->size_x; x++, output_index++) {
                    ...
                    // ogt_vox_rgba color =
                    // voxscene->palette.color[color_index];
    ...
    for (size_t i = 0; i < voxscene->num_instances; ++i) {
        ...
        /* glm::mat3x4 glmtransform = {trans.m00, trans.m01, trans.m02,
           trans.m03, trans.m10, trans.m11, trans.m12, trans.m13, trans.m20,
