...
static std::vector<uint32_t> df_256_256_256_12_svo_4_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);

static std::array<uint32_t, 2> svo_4_construct_node(Voxelizer &voxelizer, std::vector<uint32_t> &buffer, uint32_t lower_x, uint32_t lower_y, uint32_t lower_z, bool &is_empty);
...
std::vector<uint32_t> df_256_256_256_12_svo_4_construct(Voxelizer &voxelizer) {
    ...
    bool is_empty;
    auto root_node = df_256_256_256_12_svo_4_construct_node(voxelizer, buffer, 0, 0, 0, is_empty);
    ...
    for (uint32_t g_z = 0; g_z < 256; ++g_z) {
        for (uint32_t g_y = 0; g_y < 256; ++g_y) {
            for (uint32_t g_x = 0; g_x < 256; ++g_x) {
                ...
                bool sub_is_empty;
                auto sub_chunk = svo_4_construct_node(voxelizer, buffer, sub_lower_x, sub_lower_y, sub_lower_z, sub_is_empty);
                if (sub_is_empty) {
